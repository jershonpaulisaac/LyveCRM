...
function getSessionFromRequest(request: NextRequest): SessionUser {
  return {
    id: request.nextUrl.searchParams.get("userId") ?? "user_exec_aurora_1",
    role: (request.nextUrl.searchParams.get("role") as SessionUser["role"]) ?? "user",
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ tasks: scoped.tasks });
}
