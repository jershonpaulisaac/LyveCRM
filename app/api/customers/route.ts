...
function getSessionFromRequest(request: NextRequest): SessionUser {
  return {
    id: request.nextUrl.searchParams.get("userId") ?? "user_admin_aurora",
    role: (request.nextUrl.searchParams.get("role") as SessionUser["role"]) ?? "company_admin",
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ customers: scoped.customers });
}
